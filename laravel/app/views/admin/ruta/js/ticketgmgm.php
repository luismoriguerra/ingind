...
var TicketsgmgmG={id:0,persona_id:"",nombre_solicitante:"",area_id:"",nombre_solicitante_area:"",descripcion:"",fecha_pendiente:"",fecha_atencion:"",fecha_solucion:"",estado_ticket:1,estado:1}; // Datos Globales
...
*/
      ...
      $("#form_ticketgmgms_modal input[type='hidden']").not('.mant').remove();
        ...
        if(titulo=='Nuevo'){ 
           ...
           // $('#form_ticketgmgms_modal #slct_estado_ticket').val(1); 
           // $('#form_ticketgmgms_modal #txt_fecha_solucion').focus();
            //$('#form_ticketgmgms_modal #txt_responsable').focus();

        }else {

            modal.find('.modal-footer .btn-primary').text('Actualizar');
            modal.find('.modal-footer .btn-primary').attr('onClick','Editar();');
            $('#form_ticketgmgms_modal #txt_persona_id').val( TicketsgmgmG.persona_id );
            $('#form_ticketgmgms_modal #nombre_solicitante').val( TicketsgmgmG.nombre_solicitante );
            $('#form_ticketgmgms_modal #nombre_solicitante_area').val( TicketsgmgmG.nombre_solicitante_area );
            $('#form_ticketgmgms_modal #txt_area_id').val( TicketsgmgmG.area_id );
            $('#form_ticketgmgms_modal #txt_fecha_pendiente').val( TicketsgmgmG.fecha_pendiente );

            $('#form_ticketgmgms_modal #txt_descripcion').val( TicketsgmgmG.descripcion );
            $("#form_ticketgmgms_modal").append("<input type='hidden' value='"+TicketsgmgmG.id+"' name='id'>");
        }
...
BtnEditar=function(btn,id){
    ...
    TicketsgmgmG.id=id;
    TicketsgmgmG.nombre_solicitante=$(tr).find("td:eq(0)").text();
    TicketsgmgmG.nombre_solicitante_area=$(tr).find("td:eq(1)").text();
    TicketsgmgmG.persona_id=$(tr).find("td:eq(0) input[name='txt_persona_id']").val();
    TicketsgmgmG.area_id=$(tr).find("td:eq(1)").text();
    TicketsgmgmG.descripcion=$(tr).find("td:eq(2)").text();
    TicketsgmgmG.fecha_pendiente=$(tr).find("td:eq(3)").text();
    $("#BtnEditar").click();
};
