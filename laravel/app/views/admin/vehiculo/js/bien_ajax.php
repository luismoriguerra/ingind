...
var Cargos={
    AgregarEditarCargo:function(AE){
        var datos=$("#form_AccionBien").serialize().split("txt_").join("").split("slct_").join("");
        var accion="";
        if(AE==1){
            accion="bien/editar";
        }else if(AE==0){
            accion="bien/createbien";
        }
            ...
            success : function(obj) {
                ...
                if(obj.rst==1){
                    $('#t_cargos').dataTable().fnDestroy();
                     ...
                     $("#form_AccionBien select").multiselect('refresh');
                    $("#msj").html('<div class="alert alert-dismissable alert-success">'+
                                        '<i class="fa fa-check"></i>'+
                                        '<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>'+
                                        '<b>'+obj.msj+'</b>'+
                                    '</div>');
                    $('#accionBien .modal-footer [data-dismiss="modal"]').click();
                }
                else{ 
    ...
    AgregarEditarBienCaracteristica:function(AE){
        var datos=$("#form_nuevaCaracteristica").serialize().split("txt_").join("").split("slct_").join("");
        var accion="Biencaracteristica/createcaracteristica";
        if(AE==1){
            accion="Biencaracteristica/editar";
        }
            ...
            success : function(obj) {
                ...
                if(obj.rst==1){
                    ...
                    $("#msj").html('<div class="alert alert-dismissable alert-success">'+
                                        ...
                                        '<b>'+obj.msj+'</b>'+
                                    '</div>');
                    $('#nuevaCaracteristica .modal-footer [data-dismiss="modal"]').click();
                }
                else{ 
                    $.each(obj.msj,function(index,datos){
                        $("#error_"+index).attr("data-original-title",datos);
     ...
     AgregarEditarEvento:function(AE){
        var datos=$("#form_nuevoEvento").serialize().split("txt_").join("").split("slct_").join("");
        var accion="Eventocaracteristica/createvento";
        if(AE==1){
            accion="Eventocaracteristica/editar";
        }
        ...
        $.ajax({
            ...
            success : function(obj) {
                ...
                if(obj.rst==1){
                    ...
                    Cargos.EventoCaracteristica({idcaracteristica:obj.id});
                    $("#msj").html('<div class="alert alert-dismissable alert-success">'+
                                        '<i class="fa fa-check"></i>'+
                                        '<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>'+
                                        '<b>'+obj.msj+'</b>'+
                                    '</div>');
                    $('#nuevoEvento .modal-footer [data-dismiss="modal"]').click();
                }
                else{ 
                    $.each(obj.msj,function(index,datos){
                        $("#error_"+index).attr("data-original-title",datos);
    ...
    CambiarEstadoCargos:function(id,AD){
     ...
     cargarBienes:function(data,type = 0){
        //getOpciones
        $.ajax({
            url         : 'bien/cargar',
            type        : 'POST',
            ...
            success : function(obj) {
                ...
                if(obj.rst == 1){
                    if(type == 0){
                        HTMLCargarBienes(obj.datos);                        
                    }else{
                        poblateInfo(obj.datos);
                    }
                }
            },
            error: function(){
     ...
     Biencaracteristica:function(data){
        $.ajax({
            url         : 'Biencaracteristica/cargar',
            type        : 'POST',
            cache       : false,
            dataType    : 'json',
            data        : data,
            ...
            success : function(obj) {
                ...
                if(obj.rst == 1){
                    if(obj.datos.length > 0){
                        HTMLCaracteristicasBien(obj.datos);
                        $(".caracteristica").removeClass('hidden');
                    }else{
                        aviso('No se encontraron caracteristicas del bien','nuevaCaracteristica');
                        $(".caracteristica").addClass('hidden');
    ...
    EventoCaracteristica:function(data){
        $.ajax({
            url         : 'Eventocaracteristica/cargar',
            type        : 'POST',
            cache       : false,
            dataType    : 'json',
            data        : data,
            ...
            success : function(obj) {
                ...
                if(obj.rst == 1){
                    if(obj.datos.length > 0){
                         HTMLCaracteristicaEventos(obj.datos);
                         $(".evento").removeClass('hidden');
                    }else{
                        aviso('No se encontro eventos en la caracteristica','nuevoEvento');
                        $(".evento").addClass('hidden');
                    }
                }
            },
      ...
      ComfirmarEvento:function(datos){
        $.ajax({
            url         : 'Eventocaracteristica/comfirmarevento',
            type        : 'POST',
            cache       : false,
            dataType    : 'json',
            data        : datos,
