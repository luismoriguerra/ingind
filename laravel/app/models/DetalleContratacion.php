...
class DetalleContratacion extends Base
{
    ...
    {
        $sSql=" SELECT cr.id, cr.contratacion_id,cr.fecha_inicio,cr.fecha_fin,cr.fecha_aviso,cr.monto,cr.fecha_conformidad,cr.tipo,cr.texto,cr.programacion_aviso,cr.nro_doc,cr.estado
                FROM contra_reque cr
                WHERE cr.estado=1 ";
        $sSql.= $array['where'];
        $oData = DB::select($sSql);
