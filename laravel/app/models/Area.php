...
/*        if(is_array(Input::get('area_id'))){
        ...
        //3199
        $sSql = '';
        $sSql.= "SELECT rd.id norden,a.nombre area,CONCAT_WS(' ',p.nombre,p.paterno,p.materno) persona,COUNT(rd.id) as cantidad,SUM(rd.ot_tiempo_transcurrido) as total,SEC_TO_TIME(SUM(rd.ot_tiempo_transcurrido) * 60) formato FROM  tablas_relacion tr 
                INNER JOIN rutas r ON r.tabla_relacion_id=tr.id AND r.estado=1 AND r.area_id IN ($area_id) AND r.flujo_id=3636
