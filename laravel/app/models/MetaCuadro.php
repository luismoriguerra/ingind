...
class MetaCuadro extends Base
{
        ...
        public function getMetaCuadro($array )
    {
        ...
        $sql.=" SELECT 
                ...
                IF(COUNT(DISTINCT ma1.id)>1,1,COUNT(DISTINCT ma1.id))/COUNT(DISTINCT mf.id)/2 + IF(COUNT(DISTINCT md1.id)>1,1,COUNT(DISTINCT md1.id))/COUNT(DISTINCT mf.id)/2 as des_por
                FROM metas_cuadro mc
